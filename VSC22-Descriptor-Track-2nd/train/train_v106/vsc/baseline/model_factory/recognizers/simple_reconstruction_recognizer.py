...
class SimpleReconstructionRecognizer(BaseRecognizer):
    ...
    def __init__(self,
        ...
        custom_backbones = {"encoder": encoder, "decoder": decoder}
        super().__init__(
            backbone=None, cls_head=None, neck=None, train_cfg=None, test_cfg=None,
    ...
    def extract_feat(self, **kwargs):
        ...
        """
        enc_outputs = self.encoder(kwargs["x"])
        dec_outputs = self.decoder(enc_outputs["hidden"])

        feats = dict(logits=enc_outputs["logits"], dec_outputs=dec_outputs)

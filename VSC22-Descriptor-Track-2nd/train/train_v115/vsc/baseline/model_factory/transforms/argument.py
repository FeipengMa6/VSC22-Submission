...
def overlay_emoji(img, seed=None, **kwargs):
    ...
    emoji_size = np.random.uniform(0.3, 0.6)
    np.random.seed(seed)
    # axis = np.random.uniform(0, 0.8, size=2)
    axis = np.random.uniform(0, 1 - emoji_size, size=2)
...
class RandomOverlayImages(RandomStackImages):
    ...
    def _transform(self, image):
        ...
        if np.random.random() < 0.3:
            ...
            img = OverlayImage(overlay_img.resize((img.width, img.height)), opacity=opacity, overlay_size=1,
                               x_pos=0., y_pos=0, p=1.0)(img)

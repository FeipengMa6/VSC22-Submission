...
def main(args):
    ...
    with torch.no_grad():
        for batch in tqdm(infer_dataloader):
            ...
            mini_batch = 128
            flat_feature = []
            for n in range(0, flat_frames.size(0), mini_batch):
                one_flat_feature = model(return_loss=False, is_inference=True, x=flat_frames[n: n + mini_batch])
                flat_feature.append(one_flat_feature)
            flat_feature = torch.cat(flat_feature, dim=0)
            ...
            # features = concat_all_gather(out_feature)
            # features_mask = concat_all_gather(video_mask)
            # timestamps = concat_all_gather(timestamps)

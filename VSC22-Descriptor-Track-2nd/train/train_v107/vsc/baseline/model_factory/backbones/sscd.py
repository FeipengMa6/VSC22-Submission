...
class Model(nn.Module):
    def __init__(self, name: str, pool_param: float = 3.0, pool="gem", output_dim=512):
        super().__init__()
        name2dims = dict(
            resnet50=(2048, 512),
            resnext101_32x4d=(2048, 1024)
        ...
        self.embeddings = nn.Sequential(
            ...
            nn.Linear(dims[1], output_dim, bias=False)
            # L2Norm(),
        )
...
class SSCDModel(nn.Module):
    ...
    def __init__(self,
        ...
        self.model = Model(name, pool_param, pool=pool, output_dim=output_dim)
...
class ResNetModel(nn.Module):
    ...
    def __init__(self,
        ...
        self.model = Model(name, 1, pool="avg")
        setattr(self.model, "embeddings", nn.Identity())
        self.embeddings = nn.Sequential(AvgPool2d(), nn.Linear(feat_dim, 1))
    ...
    def forward(self, x):
        hidden = self.model(x)
        logits = self.embeddings(hidden).squeeze(1)
        outputs = dict(hidden=hidden, logits=logits)

...
def train_step(batch_data):
    ...
    if args.product_loss:
        match_sim = (emb_a_norm * emb_b_norm).sum(dim=2)
        entropy_loss_ = (1 - match_sim).exp().mean()
    else:
    ...
    for _b, batch in enumerate(train_loader):
        ...
        if args.fp16:
            with torch.cuda.amp.autocast():
                ...
                if args.do_fgm:
                    r_at_a = 0.1 * batch["img_a"].grad / (torch.norm(batch["img_a"].grad) + 1e-8)
                    r_at_b = 0.1 * batch["img_b"].grad / (torch.norm(batch["img_b"].grad) + 1e-8)
                    batch["img_a"] = batch["img_a"].clone() + r_at_a
                    batch["img_b"] = batch["img_b"].clone() + r_at_b
                    ici_loss_adv, entropy_loss_adv = train_step(batch)
                    loss_adv = ici_loss_adv + entropy_loss_adv

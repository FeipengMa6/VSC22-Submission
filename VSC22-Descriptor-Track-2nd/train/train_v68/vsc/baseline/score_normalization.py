...
def score_normalizev2(
    ...
    if l2_normalize:
        logger.info("L2 normalizing")
        sn_queries, sn_refs, sn_score_norm_refs = [
            transform_features(x, normalize) for x in [queries, refs, score_norm_refs]
        ]
    sn_features = np.concatenate([ref.feature for ref in score_norm_refs], axis=0)
    logger.info("Applying score normalization")
    index = CandidateGeneration(sn_score_norm_refs, MaxScoreAggregation()).index.index
    ngpu = faiss.get_num_gpus()
    co = faiss.GpuMultipleClonerOptions()
    co.shard = False
    index = faiss.index_cpu_to_all_gpus(index, co=co, ngpu=ngpu)
    k = nk  # default
    for query, sn_query in tqdm.tqdm(zip(queries, sn_queries)):
        similarity, ids = index.search(sn_query.feature, k)
        top_search = sn_features[ids]
        top_search = top_search.mean(1)
        query.feature -= top_search * beta
    for ref, sn_ref in tqdm.tqdm(zip(refs, sn_refs)):
        similarity, ids = index.search(sn_ref.feature, k)
        top_search = sn_features[ids]
        top_search = top_search.mean(1)
        ref.feature -= top_search * beta
    adapted_queries, adapted_refs = [
            transform_features(x, normalize) for x in [queries, refs]
        ]
    return adapted_queries, adapted_refs

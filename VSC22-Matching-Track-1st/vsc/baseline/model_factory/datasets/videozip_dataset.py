...
class VideoZipDataSet(torch.utils.data.Dataset):
    ...
    def __init__(
            self, vids_path, zip_prefix, width, max_frames, preprocess, original_fps=1, with_rotate=False, **kwargs):
        ...
        try:
            with ZipFile(zip_path, 'r') as handler:
                ...
                for i, img_name in enumerate(img_name_list):
                    ...
                    if self.with_rotate:
                        i_img_r = i_img.rotate(90)
                        i_img_r_tensor = self.transform(i_img_r)
                        frames_tensor[i + num_data, ...] = i_img_r_tensor
                        frames_mask[i + num_data] = 1
                        timestamps[i + num_data] = torch.tensor([i / self.original_fps, (i + 1) / self.original_fps])
...
@DATASETS.register_module()
class VideoZipDataSetV2(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        print(f"### Num vids {len(self.vids)}")
        print(f"### Samples {' '.join(self.vids[:5])}")
        print(f'#### rotate: {self.with_rotate}')
    ...
    def __getitem__(self, index):
        ...
        frames_tensor_list = []
        timestamps = []
        frames_tensor_rotate_90 = []
        frames_tensor_rotate_180 = []
        frames_tensor_rotate_270 = []
        try:
            with ZipFile(zip_path, 'r') as handler:
                img_name_list = handler.namelist()
                img_name_list = sorted(img_name_list)
                ...
                for i, img_name in enumerate(img_name_list):
                    ...
                    i_img_tensor = self.transform(i_img)
                    frames_tensor_list.append(i_img_tensor)
                    timestamps.append([i/self.original_fps, (i+1) / self.original_fps])
                    if self.with_rotate:
                        i_img_r = i_img.rotate(90)
                        i_img_r_tensor = self.transform(i_img_r)
                        frames_tensor_rotate_90.append(i_img_r_tensor)
                        i_img_r = i_img.rotate(180)
                        i_img_r_tensor = self.transform(i_img_r)
                        frames_tensor_rotate_180.append(i_img_r_tensor)
                        i_img_r = i_img.rotate(270)
                        i_img_r_tensor = self.transform(i_img_r)
                        frames_tensor_rotate_270.append(i_img_r_tensor)
        except Exception as e:
            print(e)
        record = {
            "name": vid,
            "timestamp": np.array(timestamps),
            "input": frames_tensor_list,
            "input_90": frames_tensor_rotate_90,
            "input_180": frames_tensor_rotate_180,
            "input_270": frames_tensor_rotate_270
        }
        return record

...
class VideoLmdbDataSet(torch.utils.data.Dataset):
    ...
    def __init__(
            ...
            arg_lmdb_path="/mnt/nanjing3cephfs/mmvision/tyewang/projects/datas/meta-challenge/lmdb/train_vsc"
    ):
        ...
        self.hard_pipelines = np.array([
            A.Compose([
                ...
                A.RandomResizedCrop(self.width, self.width, scale=(0.65, 1), p=1),
                A.OneOf([
                    A.GaussNoise(p=0.35),
                    A.GaussianBlur(p=0.35),
                    A.RandomScale(p=0.35),
                    A.Perspective(p=0.35),
                    A.ImageCompression(20, 100, p=0.35),
                    A.RandomSnow(p=0.35),
                ]),
                A.OneOf([
                    A.ToGray(p=0.35),
                ...
                RandomCompose([
                    A.OneOf([
                        CropAndPad(p=0.35),
                        A.CropAndPad(percent=(-0.15, 0.15), p=0.35)
                    ]),
                    A.OneOf([
                        A.ColorJitter(brightness=0.8, contrast=0.8, saturation=0.8, hue=0.2, p=0.35),
                        A.RandomBrightness((-0.2, 0.1), p=0.35),
                        A.HueSaturationValue(p=0.35),
                        A.RandomBrightnessContrast(p=0.35)
                    ]),
                    A.OneOf([
                        RandomOverlayImages(self.arg_lmdb_path, self.lmdb_size, self.width, p=0.35),
                        RandomOverlayCorners(p=0.35),
                        RandomStackImages(self.arg_lmdb_path, self.lmdb_size, self.width, p=0.35),
                        OverlayText(p=0.35),
                        OverlayEmoji(p=0.35),
                    ]),
                    A.Rotate(limit=45, border_mode=0, p=0.35)
                ], shuffle=True),
            ]),
            A.Compose([
                A.OneOf([
                ...
                RandomCompose([
                    ...
                    A.Rotate(45, border_mode=0, p=0.35)
                ], shuffle=True)
            ])
        ], dtype="object")
        self.hard_pipeline_probabilities = [0.8, 0.2]
        ...
        self.easy_pipeline = A.Compose([
            ...
            A.OneOf([
                CropAndPad(p=1),
                A.CropAndPad(percent=(-0.15, 0.15), p=1)
            ], p=0.2),
        ])  # simple argument
...
class LabelVideoLmdbDataSet(VideoLmdbDataSet):
    ...
    def __init__(
        ...
        super().__init__(vids_path, meta_path, preprocess, lmdb_path, lmdb_size, width, arg_lmdb_path)

...
class BaseRecognizer(nn.Module, metaclass=ABCMeta):
    ...
    def __init__(self,
        ...
        if backbone is not None:
            ...
            self._sub_components.append('backbone')
        if neck is not None:
            self.neck = builder.build_neck(neck)
            self._sub_components.append('neck')
        if cls_head is not None:
            self.cls_head = builder.build_head(cls_head)
            self._sub_components.append('cls_head')
        ...
        '''
        if custom_backbones:
            for key, cstmBbone in custom_backbones.items():
                setattr(self, key, builder.build_backbone(cstmBbone))
                self._sub_components.append(key)

        if custom_heads:
            for key, cstmHd in custom_heads.items():
                setattr(self, key, builder.build_head(cstmHd))
                self._sub_components.append(key)
    ...
    def init_weights(self):
        ...
        for net in self._sub_components:
            getattr(self, net).init_weights()
    ...
    def average_clip(self, cls_score, num_segs=1, mode=None):
        ...
        """
        if not mode and 'average_clips' not in self.test_cfg.keys():
            raise KeyError('"average_clips" must defined in test_cfg\'s keys')

        average_clips = self.test_cfg['average_clips'] if not mode else mode
        if average_clips not in ['score', 'prob_softmax', 'prob_sigmoid', None]:
            raise ValueError(f'{average_clips} is not supported. '
                             f'Currently supported ones are '
                             f'["score", "prob_softmax", "prob_sigmoid", None]')

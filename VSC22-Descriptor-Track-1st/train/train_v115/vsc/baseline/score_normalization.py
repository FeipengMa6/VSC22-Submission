...
def score_normalize(
    ...
    for query in queries:
        # vid = query.video_id
        # scores = np.array(vid2scores[vid])[:, None]
        ...
        top_sim = similarity[:, :k].mean(axis=1, keepdims=True)
        # assert scores.shape == scores.shape
        # top_sim = np.where((scores > 0.9) & (top_sim < 0.3), top_sim * 0.5, top_sim)

        norm_term = -beta * top_sim
